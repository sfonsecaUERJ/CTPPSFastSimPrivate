...
#include "SimTransport/HectorProducerForCTPPS/interface/HectorForCTPPS.h"
...
HectorForCTPPS::HectorForCTPPS(const edm::ParameterSet & param, bool verbosity,bool CTPPSTransport) : 
...

{
    ...
    edm::ParameterSet hector_par = param.getParameter<edm::ParameterSet>("HectorForCTPPS");
    ...
    if(m_CTPPSTransport && lengthctpps>0. ) {
        ...
        if ( m_verbosity ) LogDebug("HectorForCTPPSSetup") << "HectorForCTPPS: WARNING: lengthctpps=  " << lengthctpps;
    } 
}

HectorForCTPPS::~HectorForCTPPS(){
...
void HectorForCTPPS::clearApertureFlags(){
    m_isStoppedctpps.clear();
}

void HectorForCTPPS::clear(){
    for ( std::map<unsigned int,H_BeamParticle*>::iterator it = m_beamPart.begin(); it != m_beamPart.end(); ++it ) {
        delete (*it).second;
    };
    m_beamPart.clear();
...
void HectorForCTPPS::add( const HepMC::GenEvent * evt ,const edm::EventSetup & iSetup, CLHEP::HepRandomEngine * engine) {
        ...
        if ( (*eventParticle)->status() == 1 && (*eventParticle)->pdg_id()==2212 ){
            if ( abs( (*eventParticle)->momentum().eta())>etacut && abs( (*eventParticle)->momentum().pz())>fMomentumMin){
                ...
                if ( m_beamPart.find(line) == m_beamPart.end() ) {
                    ...
                    if(m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << " fVtxMeanX: " << fVtxMeanX << " fVtxMeanY: " << fVtxMeanY << " fVtxMeanZ: "  << fVtxMeanZ ;
                    ...
                    if(m_verbosity) { 
                        LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS:add: barcode = " << line 
                            << " status = " << g->status() 
                            << " PDG Id = " << g->pdg_id() 
                            << " mass = " << mass 
                            << " pz = " << pz 
...
void HectorForCTPPS::filterCTPPS(TRandom3* rootEngine){
    ...
    if ( m_beamPart.size() && lengthctpps>0. ) {
        ...
        for (it = m_beamPart.begin(); it != m_beamPart.end(); ++it ) {
            ...
            if(m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS:filterCTPPS: barcode = " << line;
            if ( (*m_isCharged.find( line )).second ) {
                ...
                if ( direction == 1 && m_beamlineCTPPS1 != 0 ) {
                    ...
                    if(m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS:filterCTPPS: barcode = " << line << " positive is_stop=  "<< is_stop;
                }
                    ...
                    if(m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS:filterCTPPS: barcode = " << line << " negative is_stop=  "<< is_stop;
                }
                else {
                    is_stop = true;
                    if(m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS:filterCTPPS: barcode = " << line << " 0      is_stop=  "<< is_stop;
                }

                //propagating
                m_isStoppedctpps[line] = is_stop;
                if(m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS:filterCTPPS: barcode = " << line << " isStopped=" << (*m_isStoppedctpps.find(line)).second;
                ...
                if (!is_stop) {
                    ...
                    if(m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS:filterCTPPS: barcode = " << line << " x=  "<< x1_ctpps <<" y= " << y1_ctpps;
...
int  HectorForCTPPS::getDirect( unsigned int part_n ) const {
...
void HectorForCTPPS::LorentzBoost(LorentzVector& p_out, const string& frame)
{
    // Use a matrix
    double microrad = 1.e-6;
...
HepMC::GenEvent * HectorForCTPPS::addPartToHepMC( HepMC::GenEvent * evt ){
    NEvent++;
    theCorrespondenceMap.clear();
    ...
    for (it = m_beamPart.begin(); it != m_beamPart.end(); ++it ) {
        ...
        if(m_verbosity) {
            LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS:addPartToHepMC: barcode = " << line << "\n"
                << "HectorForCTPPS:addPartToHepMC: isStoppedctpps=" << (*m_isStoppedctpps.find(line)).second;
        }
        if (!((*m_isStoppedctpps.find(line)).second)){
            ...
            if ( gpart ) {
                ...
                if(ddd != 0.) {
                    if(m_verbosity) {
                        LogDebug("HectorForCTPPSEventProcessing") <<"HectorForCTPPS:: x= "<< (*(m_xAtTrPoint.find(line))).second*0.001<< "\n"
                            <<"HectorForCTPPS:: y= "<< (*(m_yAtTrPoint.find(line))).second*0.001<< "\n"
                            <<"HectorForCTPPS:: z= "<< ddd * (*(m_direct.find( line ))).second*1000. << "\n"
                            <<"HectorForCTPPS:: t= "<< time;
                    }
                    ...
                    if (m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS:addPartToHepMC: LHCTransportLink " << theLink;
                    theCorrespondenceMap.push_back(theLink);

                    if(m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS::TRANSPORTED pz= " << gpart->momentum().pz()  
            ...
            if ( gpart ) {
                gpart->set_status( 2 );
                if(m_verbosity) LogDebug("HectorForCTPPSEventProcessing") << "HectorForCTPPS::NON-transp. pz= " << gpart->momentum().pz()  
                    << " eta= "<< gpart->momentum().eta()  << " status= "<< gpart->status();
            }
        }
    }//for 
