  ...
  for(MixCollection<PSimHit>::MixItr ihit = simHits->begin();ihit != simHits->end(); ihit++)
	...
	unsigned int detlayerId = simHit->detUnitId(); 
        ...
        if(detlayerId == 2014314496 || detlayerId == 2031091712) {
            ...
            float x_trk1 = simHit->entryPoint().x()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaX*0.01);
            float y_trk1 = simHit->entryPoint().y()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaY*0.01);
        ...
        if(detlayerId == 2014838784 || detlayerId == 2031616000) {
            ...
            // Apply position smearing
            double x_trk2 = simHit->entryPoint().x()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaX*0.01);
            double y_trk2 = simHit->entryPoint().y()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaY*0.01);
