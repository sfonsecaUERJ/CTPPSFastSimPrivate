    ...
    m_verbosity(false), fBeamMomentum(0.), fCrossAngleCorr(false), fCrossingAngle(0.)
{
  ...
  //
    ...
    // Attention: the z inside the station is in meters
...
{
    // DetId codification for PSimHit - It will be replaced by CTPPSDetId
    ...
    for(int i=0;i<(int)Trk1->NHits;i++) {
        for(int j=0;j<(int)Trk2->NHits;j++){
            if (SearchTrack(i,j,Direction,xi,t,partP,pt,thx,thy,x0,y0,xt,yt,X1d,Y1d,X2d,Y2d)) {
                ...
                //Gettting the Xi and t (squared four momentum transferred) 
